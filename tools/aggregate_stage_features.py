...
def _ensure_stage_table() -> None:
    """
    Ensure the stage_feature_table exists in the database.
    """
...
def _load_recording_subject_map(dataset_id: int) -> Dict[int, str]:
    """
    Load a mapping from recording_id to subject_id for a given dataset.

    Args:
        dataset_id (int): The dataset ID.

    Returns:
        Dict[int, str]: Mapping from recording_id to subject_id.
    """
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def compute_stage_feature_table(
    dataset_id: int,
    feature_set_id: int,
    ...
    """
    Aggregate epoch-level feature matrix to stage-level: compute median and IQR for each subject × stage × feature.
    Only include subject×stage pairs with at least min_epochs_per_stage epochs.

    Args:
        dataset_id (int): Dataset ID.
        feature_set_id (int): Feature set ID.
        limit (Optional[int]): Limit the number of recordings (for debug).
        min_epochs_per_stage (int): Minimum number of epochs per subject×stage to include.
        write_to_db (bool): Whether to write the result to the database.
        out_csv (Optional[str]): If provided, save the result as a CSV file.

    Returns:
        pd.DataFrame: Aggregated stage-level feature table.
    """
    ...
    # Count number of epochs per subject×stage
    stage_counts = (
    ...
    for feat in feature_cols:
        ...
        agg = agg.drop(columns=["q1", "q3"])
        agg = agg.merge(stage_counts, on=["subject_id", "stage"], how="left")
        # Only keep subject×stage with enough epochs
    ...
    if write_to_db:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        # Remove old results for the same dataset_id and feature_set_id
        c.execute(
...
def main() -> None:
    """
    Command-line entry point for aggregating stage-level features.
    """
    import argparse
