...
def switch_to_local_mode() -> None:
    """
    Switch the system to local mode by setting environment variables and starting the local system.
...
def switch_to_celery_mode() -> None:
    """
    Switch the system to Celery mode by clearing local executor environment variables.
...
def show_current_mode() -> None:
    """
    Display the current system mode and the number of local executor worker threads.
