...
def run(df_feat: pd.DataFrame, df_meta: pd.DataFrame, output_dir: str, **kwargs) -> str:
    ...
    """
    print(f"[feature_selection] Start feature selection experiment")
    print(f"[feature_selection] Feature matrix shape: {df_feat.shape}")
    print(f"[feature_selection] Metadata shape: {df_meta.shape}")
    ...
    print(f"[feature_selection] Feature selection completed, results saved to: {output_dir}")
    return summary
...
def preprocess_data(df_feat: pd.DataFrame, df_meta: pd.DataFrame, target_var: str) -> tuple:
    ...
    print(f"[feature_selection] Final data shape: X={X.shape}, y={y.shape}")
    return df_combined, X, y


def variance_based_selection(X: pd.DataFrame, threshold: float) -> dict:
    """Variance-based feature selection"""
    print("[feature_selection] Performing variance-based selection...")
...
def correlation_based_selection(X: pd.DataFrame, threshold: float) -> dict:
    ...
    print("[feature_selection] Performing correlation-based selection...")
...
def univariate_selection(X: pd.DataFrame, y: pd.Series, n_features: int, method: str) -> dict:
    ...
    print(f"[feature_selection] Performing {method} univariate selection...")
...
def lasso_based_selection(X: pd.DataFrame, y: pd.Series, n_features: int) -> dict:
    ...
    print("[feature_selection] Performing Lasso-based selection...")
...
def recursive_feature_elimination(X: pd.DataFrame, y: pd.Series, n_features: int) -> dict:
    """Recursive feature elimination"""
    print("[feature_selection] Performing recursive feature elimination...")
...
def principal_component_analysis(X: pd.DataFrame, n_features: int) -> dict:
    """Principal component analysis"""
    print("[feature_selection] Performing principal component analysis...")
...
def compare_selection_methods(selection_results: dict, X: pd.DataFrame, y: pd.Series) -> dict:
    ...
    print("[feature_selection] Comparing selection methods...")
    ...
    for method_name, result in selection_results.items():
        ...
        try:
            ...
            print(f"[feature_selection] Error evaluating {method_name}: {e}")
            comparison[method_name] = {
                'n_features': len(selected_features),
...
def plot_feature_importance_analysis(selection_results: dict, X: pd.DataFrame, y: pd.Series, output_dir: str):
    ...
    print("[feature_selection] Plotting feature importance analysis...")
